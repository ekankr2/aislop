...
{#if items.length > 0}
  ...
  <ol class="pt-5 pb-1">
    {#each items as item (item.slug)}
      ...
      <!-- ⚠️ 행 간격은 `0.9em`(14.4px)이다(2026-09-22 유저 지적 — "리스트가 너무
           촘촘해서 가독성이 떨어진다"). 0.55em이던 시절 행 사이가 17px이었는데,
           20px 굵은 제목이 16행 이어지면 두 줄짜리 행이 안 묶이고 글자벽이 된다.
           지금은 제목↔byline 2px 대 행↔행 29px이라 한 행이 한 덩어리로 읽힌다.
           ⚠️ 더 벌리지 마라 — 1.05em까지 올려 봤더니 행이 서로 떠서 목록을 한눈에
              훑는다는 일 자체가 안 된다. 여기는 읽는 화면이 아니라 훑는 화면이다. -->
      <li class="flex items-start gap-2.5 py-[0.9em]">
